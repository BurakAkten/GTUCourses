...
public class _mainTester {
    ...
    static void testManMap(int dataType, String filename) throws Exception {
        ...
        if (1 == dataType) {
            ...
            BigramMap<Integer> myBg = new BigramMap<Integer>(dataType);
            _mainTester.<Integer>testFunc(myBg, filename, 3, 4);		
	}
	
	else if (2 == dataType) {

            BigramMap<String> myBg = new BigramMap<String>(dataType);
            _mainTester.<String>testFunc( myBg, filename, "qwe", "asd");
	}
	else if (3 == dataType) {

            BigramMap<Double> myBg = new BigramMap<Double>(dataType);
	...
	static void testManDyn(int dataType, String filename) throws Exception {
        ...
        if (1 == dataType) {
            ...
            BigramDyn<Integer> myBg = new BigramDyn<Integer>(dataType);
            _mainTester.<Integer>testFunc(myBg, filename, 1, 4);		
	}
	
	else if (2 == dataType) {

            BigramDyn<String> myBg = new BigramDyn<String>(dataType);
            _mainTester.<String>testFunc( myBg, filename, "qwe", "asd");
	}
	else if (3 == dataType) {

            BigramDyn<Double> myBg = new BigramDyn<Double>(dataType);
            _mainTester.<Double>testFunc(myBg, filename, 3.0, 4.0);	
	}        
}
